...
export async function createResume(input: CreateResumeInput): Promise<Resume> {
  ...
  const query = `
    INSERT INTO resumes (user_id, full_name, occupation, description, education, experience, published)
...
export async function getResumeById(id: string): Promise<Resume> {
  ...
  const query = `SELECT id, user_id, full_name, occupation, description, education, experience, published, created_at, updated_at FROM resumes WHERE id = $1`;
...
export async function getAllResumes(): Promise<Resume[]> {
  const pool = getDbPool();
  const query = `SELECT id, user_id, full_name, occupation, description, education, experience, published, created_at, updated_at FROM resumes`;
  const result = await pool.query<ResumeRow>(query);
